 ...
 */
const { callWithRotation, GEMINI_MODEL } = require('../services/geminiService');
...
async function extractTransactions(text, onUsage, onChunkProgress) {
  ...
  // 2. Run AI chunked extraction with optimized chunk size (12,000 chars) for fast Gemini Flash processing
  ...
  console.log(`[banking] Text length ${cleanedText.length} divided into ${chunks.length} chunks (chunk size ${CHUNK_SIZE})`);
...
Return JSON array only:`;
      ...
      try {
        const raw = await callWithRotation(() => [{ text: prompt }], 16384, GEMINI_MODEL, onUsage);
        ...
        if (typeof onChunkProgress === 'function') {
          try {
            onChunkProgress(completedChunks, chunks.length, aiTransactions.length);
          } catch (_) {}
        }
      }
    }));
...
Be specific with numbers. Be concise but thorough.`;
  ...
  return callWithRotation(() => [{ text: prompt }], 3000, GEMINI_MODEL, onUsage);
}
...
Answer:`;
    ...
    const res = await callWithRotation(() => [{ text: prompt }], 2048, GEMINI_MODEL, onUsage);
